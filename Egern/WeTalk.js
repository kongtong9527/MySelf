//2026/04/21
// Adapted for Egern from Quantumult X
...
async function fetchApi(ctx, path, capture, headers) {
  const url = buildUrl(path, capture);
  const resp = await ctx.http.get(url, { headers });
  return await resp.text();
}

async function doVideoLoop(ctx, count, capture, headers, msgs) {
  for (let i = 0; i < count; i++) {
    if (i > 0) await sleep(VIDEO_DELAY);
    else await sleep(1500);
    try {
      const body = await fetchApi(ctx, 'videoBonus', capture, headers);
      const d = JSON.parse(body);
      if (d.retcode === 0) {
        msgs.push(`🎬 视频${i+1}：+${d.result?.bonus || '?'} Coins`);
      } else {
        msgs.push(`⏸ 视频${i+1}：${d.retmsg}`);
        break;
      }
    } catch (e) {
      msgs.push(`❌ 视频${i+1}：解析失败`);
      break;
    }
  }
}
...
async function runAccount(ctx, acc, index, total) {
  ...
  try {
    let body = await fetchApi(ctx, 'queryBalanceAndBonus', acc.capture, headers);
    try {
      const d = JSON.parse(body);
    ...
    body = await fetchApi(ctx, 'checkIn', acc.capture, headers);
    try {
      const d = JSON.parse(body);
    ...
    await doVideoLoop(ctx, MAX_VIDEO, acc.capture, headers, msgs);

    body = await fetchApi(ctx, 'queryBalanceAndBonus', acc.capture, headers);
    try {
      const d = JSON.parse(body);
      if (d.retcode === 0) msgs.push(`💰 最新余额：${d.result.balance} Coins`);
    } catch (e) {}
  } catch (err) {
    msgs.push(`❌ 异常：${String(err)}`);
  }
  return msgs.join('\n');
}

export default async function(ctx) {
  // http_request mode: capture account data from intercepted request
  if (ctx.request) {
    ...
    store.accounts[fp] = {
      ...
      uaSeed,
      baseUA,
      capture: { url: ctx.request.url, paramsRaw, headers: headersMap },
    ...
    ctx.notify({ title: scriptName, subtitle, body: `${alias}（id:${fp}）\n当前账号总数：${total}` });
  ...
  if (!ids.length) {
    ctx.notify({ title: scriptName, subtitle: '⚠️ 未抓到任何账号', body: '请先打开 WeTalk 触发抓包' });
    return;
  }
  ...
  ctx.notify({ title: scriptName, subtitle: `🎉 全部完成 (${total}个账号)`, body: results.join('\n———\n') });
}
